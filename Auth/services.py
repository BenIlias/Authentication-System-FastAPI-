...
class AuthService():
    ...
    def login(self, user_data: LoginUser):
        ...
        if user_db.token.revoked:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='You have no access')
        ...
        tokens = Token_list(user=user_db, refresh_token=refresh_token, access_token=access_token)
        

        if user_db.token.access_token:
            user_db.token.access_token = access_token
            user_db.token.refresh_token = refresh_token
            self.repository.update_token(user_db)
            return {'access_token': access_token, 'refresh_token': refresh_token}


        
        
        self.repository.add_token(tokens)
        return {'access_token': access_token, 'refresh_token': refresh_token}


    def get_profile(self, token: str):
        username = verify_token(Token_type.access_token.value, token)
        if self.repository.get_user_by_username(username).token.revoked:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='token is rovoked')
